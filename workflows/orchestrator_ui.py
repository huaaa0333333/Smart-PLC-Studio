...
def render(client, collection):
    ...
    if t_col1.button("🟢 簡易輸送帶邏輯", use_container_width=True):
         st.session_state.prompt_template = "設計一個帶有啟動停按鈕與馬達的簡單輸送帶系統。條件：要有綠色運轉燈，還要能按急停按鈕讓整台機器瞬間停下來。預算不高，越簡單越好。"
    if t_col2.button("🌡️ PID 恆溫水槽控制", use_container_width=True):
         st.session_state.prompt_template = "這是一個水循環加熱系統，請幫我設計一個 PID_Compact 溫控邏輯。輸入是 PT100 溫度計，輸出是 PWM 加熱棒控制，並包含上下限警報。"
    if t_col3.button("⚙️ 汽缸順序位移控制", use_container_width=True):
         st.session_state.prompt_template = "幫我寫一個順序控制邏輯 (Sequence Control)。按下啟動後，A汽缸伸出 -> A到位後B汽缸伸出 -> B到位後A退回 -> A退回到位後B退回。需要防呆與時間監控。"
    ...
    if st.button("🚀 啟動無縫資料流生產線", type="primary", use_container_width=True):
        ...
        # ==========================================
        # 絕美展示：使用 Tabs 頁籤分門別類展示多代理人成果
        # ==========================================
        st.success("🎉 全部生成完畢！請使用下方頁籤切換查看報告：")
        
        # 建立三個 Tabs
        tab1, tab2, tab3 = st.tabs(["📋 系統硬體與架構配置", "💻 核心邏輯與 SCL 程式碼", "🖥️ HMI 人機介面佈局"])
                ...
                with dcol2:
                    try:
                        ...
                        st.download_button(
                            ...
                            file_name=f"Automated_Tags_{target_version}.xlsx", 
